...
plt1 = annotate!(plt1, 35, 1.8, Plots.text(@sprintf("\$ \\varepsilon = %0.1f\$",ε), :right, :middle))
...
anim = @animate for it = vcat(ones(Int64,4,1),collect(Int64,1:50:nt),nt*ones(Int64,10,1))
    ...
    pltA = plot!(pltA, [time_sim[it]], [x[it]]; label="", marker=:circle, mfc=:orange, ms=6, xlims=(-2,tend+2), ylims=(-2.2,2.2))
    pltA = annotate!(pltA, 17, -1.4, Plots.text(@sprintf("\$ \\varepsilon = %0.1f\$",ε), :right, :middle))
    pltA = annotate!(pltA, 17, -1.8, Plots.text(@sprintf("\$ x _0 = %0.2f\$",x₀), :right, :middle))
    ...
    pltB = plot!(pltB, [x[it]],[v[it]]; label="", marker=:circle, mfc=:orange, ms=6, xlims=(-2.2,2.2), ylims=(-15.2,15.2))
    pltG = plot(pltB, pltA, layout=(1,2), size=(1200,600), margin=5*Plots.PlotMeasures.mm)
end

## save
gifname = "test_vanderPol_eps2.gif"
